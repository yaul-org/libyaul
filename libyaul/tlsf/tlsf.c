...
{
        const tlsfptr_t aligned = (tlsf_cast(tlsfptr_t, ptr) + \
            (align - 1)) & ~(align - 1);
        tlsf_assert(0 == (align & (align - 1)), "must align to a power of two");
...
static block_header_t *
search_suitable_block(pool_t *pool, int *fli, int *sli)
{
        int fl = *fli;
        int sl = *sli;
...
static void
remove_free_block(pool_t *pool, block_header_t *block, int fl, int sl)
{
...
static void
insert_free_block(pool_t *pool, block_header_t *block, int fl, int sl)
{
...
static void
block_remove(pool_t *pool, block_header_t *block)
{
        int fl, sl;
...
static void
block_insert(pool_t *pool, block_header_t *block)
{
        int fl, sl;
        mapping_insert(block_size(block), &fl, &sl);
...
static block_header_t *
block_merge_prev(pool_t *pool, block_header_t *block)
{
...
static block_header_t *
block_merge_next(pool_t *pool, block_header_t *block)
{
...
static void
block_trim_free(pool_t *pool, block_header_t *block, size_t size)
{
...
static block_header_t *
block_trim_free_leading(pool_t *pool, block_header_t *block, size_t size)
{
...
static block_header_t *
block_locate_free(pool_t *pool, size_t size)
{
        int fl = 0, sl = 0;
...
static void *
block_prepare_used(pool_t *pool, block_header_t *block, size_t size)
{
        void *p = 0;
...
#define tlsf_insist(x, s) { tlsf_assert(x, s); if (!(x)) { status--; } }
...
/* Returns non-zero if heap check fails. */
int
tlsf_check_heap(tlsf_pool tlsf)
{
        int i, j;
        ...
        for (i = 0; i < FL_INDEX_COUNT; ++i) {
                for (j = 0; j < SL_INDEX_COUNT; ++j) {
                        ...
                        if (!sl_map) {
                                tlsf_insist(block == &pool->block_null,
...
void
tlsf_walk_heap(tlsf_pool pool, tlsf_walker walker, void *user)
{
 ...
 */
/* Create or destroy a memory pool. */
tlsf_pool
...
void *
tlsf_malloc(tlsf_pool tlsf, size_t size)
{
        pool_t *pool = tlsf_cast(pool_t *, tlsf);
...
void *
tlsf_memalign(tlsf_pool tlsf, size_t align, size_t size)
{
        pool_t *pool = tlsf_cast(pool_t *, tlsf);
        const size_t adjust = adjust_request_size(size, ALIGN_SIZE);
...
void
tlsf_free(tlsf_pool tlsf, void *ptr)
{
 ...
 */
void *
tlsf_realloc(tlsf_pool tlsf, void *ptr, size_t size)
{
        pool_t *pool = tlsf_cast(pool_t *, tlsf);
        void *p = 0;
