...
void
fix16_matrix4_zero(fix16_matrix4_t *m0)
{
        memset(&m0->arr[0], 0, sizeof(fix16_t) * 4 * 4);
}

void
fix16_matrix4_dup(const fix16_matrix4_t *m0, fix16_matrix4_t *result)
{
        (void)memcpy(result, m0, sizeof(fix16_matrix4_t));
}

void
fix16_matrix4_identity(fix16_matrix4_t *m0)
{
        fix16_matrix4_zero(m0);

        m0->frow[0][0] = F16(1.0f);
        m0->frow[1][1] = F16(1.0f);
        m0->frow[2][2] = F16(1.0f);
        m0->frow[3][3] = F16(1.0f);
}

void
fix16_matrix4_multiply(const fix16_matrix4_t *m0, const fix16_matrix4_t *m1,
    fix16_matrix4_t *result)
{
...
void
fix16_matrix4_multiply2(const fix16_matrix4_t *m0, const fix16_matrix4_t *m1,
    fix16_matrix4_t *result)
{
#define DOT_PRODUCT(i, j) fix16_add(                                           \
                fix16_add(fix16_mul(m0->frow[i][0], m1->frow[0][j]),           \
        ...
        result->frow[0][1] = DOT_PRODUCT(0, 1);
        result->frow[0][2] = DOT_PRODUCT(0, 2);
        result->frow[0][3] = F16(0.0f);

        result->frow[1][0] = DOT_PRODUCT(1, 0);
        result->frow[1][1] = DOT_PRODUCT(1, 1);
        result->frow[1][2] = DOT_PRODUCT(1, 2);
        result->frow[1][3] = F16(0.0f);

        result->frow[2][0] = DOT_PRODUCT(2, 0);
        result->frow[2][1] = DOT_PRODUCT(2, 1);
        result->frow[2][2] = DOT_PRODUCT(2, 2);
        result->frow[2][3] = F16(0.0f);

        result->frow[3][0] = DOT_PRODUCT(3, 0);
        result->frow[3][1] = DOT_PRODUCT(3, 1);
        result->frow[3][2] = DOT_PRODUCT(3, 2);
        result->frow[3][3] = F16(1.0f);
}

void
fix16_matrix4_transpose(const fix16_matrix4_t *m0, fix16_matrix4_t *result)
{
...
void
fix16_matrix4_inverse(const fix16_matrix4_t *m0, fix16_matrix4_t *result)
{
        /* Only applicable to affine transformation matrix */

        /* Last row: [0 0 0 1] */
        result->frow[3][0] = F16(0.0f);
        result->frow[3][1] = F16(0.0f);
        result->frow[3][2] = F16(0.0f);
        result->frow[3][3] = F16(1.0f);

        /* Transpose 3x3 sub-matrix */
        result->frow[0][0] = m0->frow[0][0];
        result->frow[0][1] = m0->frow[1][0];
        result->frow[0][2] = m0->frow[2][0];
...
void
fix16_matrix4_str(const fix16_matrix4_t *m0, char *buf, int decimals)
{
        ...
        for (row_idx = 0; row_idx < 4; row_idx++) {
                ...
                *buf++ = '|';
                buf_start = buf;
                buf_end = buf;
                fix16_vector4_str(&m0->row[row_idx], buf, decimals);
