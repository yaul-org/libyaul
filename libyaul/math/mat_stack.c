...
void
mat_stack_init(mat_stack_t *mat_stack, fix16_mat43_t *matrix_pool, uint32_t count)
{
    ...
    for (uint32_t i = 0; i < mat_stack->stack_count; i++) {
        fix16_mat43_identity(&mat_stack->top_matrix[i]);
    }
}
...
mat_stack_push(mat_stack_t *mat_stack)
{
    ...
    const fix16_mat43_t * const prev_matrix = mat_stack->top_matrix - 1;

    fix16_mat43_dup(prev_matrix, mat_stack->top_matrix);
}
...
void
mat_stack_set(mat_stack_t *mat_stack, const fix16_mat43_t *m0)
{
    assert(mat_stack != NULL);

    fix16_mat43_dup(m0, mat_stack->top_matrix);
}

void
mat_stack_copy(mat_stack_t *mat_stack, fix16_mat43_t *m0)
{
    assert(mat_stack != NULL);

    fix16_mat43_dup(mat_stack->top_matrix, m0);
}
...
mat_stack_identity(mat_stack_t *mat_stack)
{
    ...
    fix16_mat43_identity(mat_stack->top_matrix);
}
...
mat_stack_x_translate(mat_stack_t *mat_stack, fix16_t x)
{
    ...
    fix16_mat43_t * const top_matrix = mat_stack->top_matrix;
