    ...
    const vdp2_scrn_rotation_map_t *rotation_map)
{
        ...
        if (rotation_map->single) {
                ...
                mpab_reg = (uint16_t *)&_state_vdp2()->regs->mpabra;

                uint16_t *mp_reg;
                mp_reg = (uint16_t *)&_state_vdp2()->regs->mpcdra;
...
vdp2_scrn_rotation_rp_mode_set(const vdp2_scrn_rotation_params_t *rotation_params)
{
        ...
        _state_vdp2()->regs->rpmd |= rotation_params->rp_mode;
}

void
vdp2_scrn_rotation_rp_table_set(const vdp2_scrn_rotation_params_t *rotation_params)
{
        _state_vdp2()->regs->rptau = VDP2_VRAM_BANK(rotation_params->rp_table_base);
        _state_vdp2()->regs->rptal = rotation_params->rp_table_base & 0xFFFE;
}

void
vdp2_scrn_rotation_coeff_table_set(const vdp2_scrn_rotation_params_t *rotation_params)
{
        _state_vdp2()->regs->ktaof = 0x0000;
        ...
        case VDP2_SCRN_COEFF_WORD_SIZE_1:
                _state_vdp2()->regs->ktaof = (rotation_params->rp_table->kast >> 17) & 0x07;
                break;
        case VDP2_SCRN_COEFF_WORD_SIZE_2:
                _state_vdp2()->regs->ktaof = (rotation_params->rp_table->kast >> 18) & 0x07;
                break;
        default:
                break;
        }
...
vdp2_scrn_rotation_sop_set(const vdp2_scrn_rotation_params_t *rotation_params)
{
        ...
        case VDP2_SCRN_RP_MODE_0:
                _state_vdp2()->regs->plsz &= 0xF3FF;
                _state_vdp2()->regs->plsz |= rotation_params->rsop_type << 10;
                break;
        case VDP2_SCRN_RP_MODE_1:
                _state_vdp2()->regs->plsz &= 0x3FFF;
                _state_vdp2()->regs->plsz |= rotation_params->rsop_type << 14;
                break;
...
vdp2_scrn_rotation_rp_read_set(const vdp2_scrn_rotation_params_t *rotation_params)
{
        ...
        switch (rotation_params->rp_mode) {
        case VDP2_SCRN_RP_MODE_0:
                _state_vdp2()->regs->rprctl &= 0xFFF8;

                if (coeff_params->enable) {
                        _state_vdp2()->regs->rprctl |=  coeff_params->xst_read_enable |
                                                       (coeff_params->yst_read_enable << 1) |
                                                       (coeff_params->kast_read_enable << 2);
                }
                break;
        case VDP2_SCRN_RP_MODE_1:
                _state_vdp2()->regs->rprctl &= 0xF8FF;

                if (coeff_params->enable) {
                        _state_vdp2()->regs->rprctl |= (coeff_params->xst_read_enable << 8) |
...
vdp2_scrn_rotation_coeff_params_set(const vdp2_scrn_rotation_params_t *rotation_params)
{
        ...
        switch (rotation_params->rp_mode) {
        case VDP2_SCRN_RP_MODE_0:
                _state_vdp2()->regs->ktctl &= 0xFFFF;

                if (coeff_params->enable) {
                        _state_vdp2()->regs->ktctl |= (coeff_params->enable << 0) |
        ...
        case VDP2_SCRN_RP_MODE_1:
                ...
                if (coeff_params->enable) {
                        _state_vdp2()->regs->ktctl |= (coeff_params->enable << 8) |
